...
def _load_rewrites(request: Request, session_id: int) -> list[Rewrite]:
    with closing(request.app.state.db_connect()) as conn:
        rows = conn.execute(
            "SELECT gap_index, action, original_bullet, rewritten_bullet, "
            "sources_json, reason FROM optimise_rewrites "
            "WHERE session_id = ? ORDER BY id ASC",
            (session_id,),
        ).fetchall()
    return [
        Rewrite(
            ...
            sources=[RewriteSource(**s) for s in json.loads(r["sources_json"])],
            reason=r["reason"],
        )
        for r in rows
    ]
...
def send_message(
    ...
    try:
        ...
        _record_skip(
            ...
            current_index,
            len(gaps),
            reason=f"Could not fill this gap without inventing information: {exc}",
            assistant_message="I could not close this gap without inventing details, so I skipped it.",
        )
    except OptimiseError as exc:
        raise HTTPException(status_code=502, detail=f"Optimise call failed: {exc}") from exc
    else:
        _apply_outcome(request, session_id, current_index, len(gaps), outcome)
...
def _record_skip(
    ...
    *,
    ...
    """Persist a skip row, its assistant message, and advance the pointer."""
    _persist_rewrite(
        request,
        session_id,
        gap_index,
