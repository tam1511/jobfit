...
Anti-fabrication is enforced in code, not just prompt. The model returns a `sources` array of verbatim substrings from the CV or the user's messages; `verify_grounding()` then checks (1) each source really appears in its declared origin, (2) any model-supplied `original_bullet` is verbatim from the CV — so the model cannot invent a plausible "original" containing a fake metric and pass by "preserving" it — and (3) every hard fact in the rewrite (numbers with units via `_NUMBER_RE`, currency, CamelCase / ALLCAPS tokens via `_TOKEN_RE` minus a small `_TOKEN_STOPWORDS` list of ordinary verbs) is present in a valid source or preserved verbatim from the original bullet. Failures raise `FabricationError`, which the router converts into a "cannot fill this gap" skip and moves on. The product never salvages by rephrasing.
...
- `POST /api/optimise/{session_id}/message` — user reply; the outcome may be ask / rewrite / skip. `_advance()` uses a conditional `UPDATE ... WHERE current_gap_index = ?` for race-safety and `_walked_gaps()` filters low + sorts high-before-medium in one pass.
...
Session state survives container restarts via three new tables — `optimise_sessions` (one per upload), `optimise_messages`, `optimise_rewrites` — added by an additive `v2 → v3` migration in `_apply_migrations`. `create_app(..., optimise_fn=None)` accepts an injected rewriter for tests.

Frontend: `OptimiseDrawer` calls `startOptimise()` on mount, then `sendOptimiseMessage()` / `skipOptimiseGap()`, auto-scrolls on new messages, and is mounted from both `/app/new` and `/app/applications`. `ScorePanel` computes `walkableGaps = high + medium` and hides the Optimise button entirely when the list is empty.

Tests (134 pass, 3 skipped): `test_optimise_grounding.py` locks the guard's behaviour (grounded from CV or user, preserved metric needs no source, invented numbers / tool names rejected, sources-not-actually-in-origin rejected, common verbs not flagged). `test_optimise_unit.py` covers `rewrite()`: ask outcome, grounded rewrite, fabricated metric → `FabricationError`, missing api_key fails fast without calling the stub, retry-once, gives-up-after-second-failure, missing required field. `test_optimise_endpoint.py` covers auth, ownership, low-gap filtering, high-before-medium ordering, 409 no-walkable-gaps, start idempotency, ask-does-not-advance, rewrite-persists-and-advances, model-skip records reason, `FabricationError` → unfillable skip, user-skip advances without a model call, completing the last gap marks the session done, blank message → 422, OpenRouter error → 502. `test_db_init.py` gains a v2 → v3 migration case that asserts the three optimise tables exist and `user_version == 3`.